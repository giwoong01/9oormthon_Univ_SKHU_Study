...
import com.skhu.goormthon.app.dto.auth.reqDto.EmailSendReqDto;
import com.skhu.goormthon.app.dto.auth.reqDto.EmailVerifyReqDto;
    ...
    private MimeMessage createMessage(String to, String code) throws MessagingException, UnsupportedEncodingException {
        ...
        String msgContent = """
                <div style='margin: 0 auto; padding: 20px; max-width: 600px; font-family: Arial, sans-serif;'>
                    ...
                    <p style='text-align: center; color: #999;'>감사합니다.<br>goormthon 팀</p>
                </div>
                """.formatted(code);

        message.setText(msgContent, "utf-8", "html");
        message.setFrom(new InternetAddress("kduoh99@gmail.com", "goormthon"));

        return message;
    }

    @Transactional
    public ApiResTemplate<Void> sendVerificationCode(EmailSendReqDto reqDto) {
        ...
        return ApiResTemplate.success(SuccessCode.EMAIL_SEND_CODE_SUCCESS, null);
    }

    @Transactional
    public ApiResTemplate<Void> verifyCode(EmailVerifyReqDto reqDto) {
        ...
        return ApiResTemplate.success(SuccessCode.EMAIL_VERIFICATION_SUCCESS, null);
    }
